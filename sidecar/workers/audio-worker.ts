  ...
  async run(ctx: JobContext, onProgress: ProgressCallback): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        /* bs1770gain não instalado — não crítico */
      }
      ...
      emit({
        ...
        level: 'INFO',
        source: 'audio-worker',
        message: `Audio completed: ${after.integratedLufs.toFixed(1)} LUFS -> ${finalPath}`,
      });
...
async function loudnessAnalysis(ffmpegPath: string, input: string): Promise<LoudnessResult> {
  ...
  const match = stderr.match(
    /\{[\s\S]*?"input_i"\s*:\s*"([-\d.]+)"[\s\S]*?"input_tp"\s*:\s*"([-\d.]+)"[\s\S]*?"input_lra"\s*:\s*"([-\d.]+)"/,
  );
  if (match) {
    return {
...
async function loudnessNormalize(
  ...
  await execFileAsync(
    ...
    [
      ...
      input,
      '-af',
      `volume=${gainDb.toFixed(2)}dB,alimiter=level_in=1:level_out=1:limit=${Math.pow(10, truePeakLimit / 20).toFixed(4)}:attack=5:release=50`,
      '-ar',
      '48000',
      '-y',
      output,
