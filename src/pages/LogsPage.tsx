    ...
    <div className="flex flex-col h-full animate-in fade-in duration-300">
      ...
      <div className="flex items-center justify-between mb-6 shrink-0">
        <div>
          ...
          <div className="flex items-center gap-2 text-xs font-medium">
            <span className="px-2 py-1 bg-bg-secondary border border-border rounded-md text-text-secondary">
              {t('logs.total', { count: totalLogs })}
            </span>
            {errorLogs > 0 && (
              <span className="px-2 py-1 bg-red-500/10 border border-red-500/20 rounded-md text-red-400">
                {t('logs.errors')}: {errorLogs}
              </span>
            )}
            {warnLogs > 0 && (
              <span className="px-2 py-1 bg-yellow-500/10 border border-yellow-500/20 rounded-md text-yellow-400">
                {t('logs.warnings')}: {warnLogs}
              </span>
            )}
          </div>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={handleClear}
            className="flex items-center gap-2 px-4 py-2 bg-bg-secondary border border-border hover:bg-red-500/20 hover:text-red-400 hover:border-red-500/30 text-text-secondary rounded-lg text-sm font-medium transition-colors"
          >
      ...
      <div className="flex flex-wrap items-center gap-4 bg-bg-secondary border border-border rounded-xl p-4 mb-6 shrink-0">
        ...
        <select
          ...
          className="bg-bg-primary border border-border rounded-lg px-4 py-2 text-sm text-text-primary outline-none"
        >
          <option>{t('logs.sourceAll')}</option>
        ...
        <select
          ...
          className="bg-bg-primary border border-border rounded-lg px-4 py-2 text-sm text-text-primary outline-none"
        >
          <option>{t('logs.timeAll')}</option>
          <option>{t('logs.time1h')}</option>
          <option>{t('logs.time6h')}</option>
          <option>{t('logs.time24h')}</option>
          <option>{t('logs.time7d')}</option>
        </select>
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto p-2">
          {filteredLogs.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-text-muted">
              ...
              <p>{t('logs.noLogs')}</p>
              {(levelFilter !== t('logs.allLevels') ||
                sourceFilter !== t('logs.sourceAll') ||
                searchTerm) && <p className="text-xs mt-2">{t('logs.tryRemoveFilters')}</p>}
            </div>
          ) : (
            [...filteredLogs].reverse().map((log) => {
              ...
              return (
                ...
                >
                  <div className="font-mono text-xs text-text-secondary flex items-start mt-0.5">
                    {time}
                  </div>
                  <div className="flex items-start">
                    <span
                      className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${getLevelColor(log.level)}`}
                    >
                      {log.level}
                    </span>
                  </div>
                  ...
                  <div
                    className={`text-text-secondary font-mono text-xs whitespace-pre-wrap ${getRowBorder(log.level)}`}
                  >
