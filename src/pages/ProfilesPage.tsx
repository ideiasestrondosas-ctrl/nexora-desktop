...
export default function ProfilesPage() {
  ...
  const fetchProfiles = async () => {
    try {
      ...
      const fallback = [
        { ...DEFAULT_PROFILE, id: 'broadcast-hd', name: 'broadcast-hd', is_system: true, description: t('profiles.fallback.broadcastHd'), resolution: '1920x1080', video_bitrate_k: 50000 },
        { ...DEFAULT_PROFILE, id: 'broadcast-sd', name: 'broadcast-sd', is_system: true, description: t('profiles.fallback.broadcastSd'), resolution: '720x576', video_bitrate_k: 15000 },
        ...
        { ...DEFAULT_PROFILE, id: 'web-hd', name: 'web-hd', is_system: true, description: t('profiles.fallback.webHd'), resolution: '1920x1080', video_bitrate_k: 8000 },
        { ...DEFAULT_PROFILE, id: 'proxy', name: 'proxy', is_system: true, description: t('profiles.fallback.proxy'), resolution: '1280x720', video_bitrate_k: 2000 },
        { ...DEFAULT_PROFILE, id: 'social', name: 'social', is_system: true, description: t('profiles.fallback.social'), resolution: '1080x1920', video_bitrate_k: 4000 },
      ];
  ...
  const handleCreateNew = () => {
    setEditingProfile({ ...DEFAULT_PROFILE, id: `custom-${Date.now()}`, name: t('profiles.newProfile'), description: t('profiles.defaultDescription') });
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-8 shrink-0">
        ...
        <div className="relative flex-1 min-w-0">
          ...
          >
            ...
            <ChevronDown size={16} className={cn('text-text-muted shrink-0 transition-transform', dropdownOpen && 'rotate-180')} />
          </button>
          ...
          {dropdownOpen && (
            <>
              ...
              <div className="absolute top-full left-0 right-0 mt-2 bg-bg-secondary border border-border rounded-xl shadow-2xl z-40 max-h-72 overflow-y-auto">
                {/* Sistema */}
                <div className="px-3 py-2 text-[10px] font-black uppercase tracking-widest text-text-muted">{t('profiles.predefined')}</div>
                ...
                {profiles.some(p => !p.is_system) && (
                  <>
                    <div className="px-3 py-2 text-[10px] font-black uppercase tracking-widest text-text-muted border-t border-border">{t('profiles.custom')}</div>
                    {profiles.filter(p => !p.is_system).map(p => (
                      <button
                        key={p.id}
                        onClick={() => { setSelectedProfileId(p.id); setDropdownOpen(false); }}
        ...
        <div className="flex-1 overflow-y-auto space-y-6 pb-8">
          ...
          <div className="bg-bg-secondary border border-border rounded-2xl p-6">
            ...
            <div className="flex flex-wrap gap-2">
              <span className="px-2.5 py-1 bg-bg-primary border border-border rounded-lg text-xs font-bold text-text-secondary">{selectedProfile.container}</span>
              <span className="px-2.5 py-1 bg-bg-primary border border-border rounded-lg text-xs font-bold text-text-secondary">{selectedProfile.video_codec}</span>
              <span className="px-2.5 py-1 bg-bg-primary border border-border rounded-lg text-xs font-bold text-text-secondary">{selectedProfile.resolution}</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-bg-secondary border border-border rounded-2xl p-6">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-xs font-black uppercase tracking-widest text-text-muted">{t('profiles.video')}</h3>
              </div>
              <div className="space-y-3">
                ...
                <PropertyRow label={t('profiles.fps')} value={selectedProfile.fps?.toString() ?? t('profiles.original')} />
                <PropertyRow label={t('profiles.bitrate')} value={selectedProfile.video_bitrate_k === 0 ? t('profiles.auto') : `${(selectedProfile.video_bitrate_k / 1000).toFixed(1)} Mbps`} />
                <PropertyRow label={t('profiles.cpuPreset')} value={selectedProfile.cpu_preset} />
                <PropertyRow label={t('profiles.h264Profile')} value={`${selectedProfile.h264_profile} @ ${selectedProfile.h264_level}`} />
              </div>
            </div>
            ...
            <div className="bg-bg-secondary border border-border rounded-2xl p-6">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-xs font-black uppercase tracking-widest text-text-muted">{t('profiles.audio')}</h3>
              </div>
              <div className="space-y-3">
                <PropertyRow label={t('profiles.audioCodec')} value={selectedProfile.audio_codec} />
                <PropertyRow label={t('profiles.audioBitrate')} value={`${selectedProfile.audio_bitrate_k} kbps`} />
                <PropertyRow label={t('profiles.sampleRate')} value={`${selectedProfile.audio_sample_rate} Hz`} />
                <PropertyRow label={t('profiles.targetLufs')} value={`${selectedProfile.target_lufs} LUFS`} />
                <PropertyRow label={t('profiles.truePeak')} value={`${selectedProfile.true_peak_limit_dbtp} dBTP`} />
              </div>
            </div>
            ...
            <div className="bg-bg-secondary border border-border rounded-2xl p-6">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-xs font-black uppercase tracking-widest text-text-muted">{t('profiles.quality')}</h3>
              </div>
              <div className="space-y-3">
                <PropertyRow label={t('profiles.vmafMin')} value={`${selectedProfile.vmaf_threshold}`} />
                <PropertyRow label={t('profiles.acceleration')} value={selectedProfile.is_system ? t('profiles.accelerationDefault') : t('profiles.accelerationCustom')} />
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="fixed right-0 top-0 bottom-0 w-[420px] bg-bg-secondary border-l border-border z-50 flex flex-col shadow-2xl animate-in slide-in-from-right">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-8">
              ...
              <div className="space-y-4">
                <h3 className="text-sm font-bold text-text-secondary uppercase tracking-wider border-b border-border pb-2">{t('profiles.general')}</h3>
                <div>
                  <label className="block text-xs font-medium text-text-secondary mb-1">{t('common.name')}</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-medium text-text-secondary mb-1">{t('profiles.container')}</label>
                  <select
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-medium text-text-secondary mb-1">{t('profiles.codec')}</label>
                    <select
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-xs font-medium text-text-secondary mb-1">{t('profiles.fps')}</label>
                    <select
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-medium text-text-secondary mb-1">{t('profiles.audioCodec')}</label>
                    <select
