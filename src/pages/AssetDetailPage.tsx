  ...
  if (!asset) {
    return (
      <div className="h-full flex flex-col items-center justify-center text-text-muted gap-4">
        ...
        <button onClick={onBack} className="px-4 py-2 bg-surface hover:bg-surface-hover rounded-xl transition-colors">
    ...
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-24">
      ...
      <div className="flex border-b border-border gap-2 shrink-0">
        ...
        >
          ...
          <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold ml-1 transition-colors ${
            activeDetailTab === 'history' ? 'bg-brand/20 text-brand' : 'bg-surface text-text-muted'
          }`}>
            {jobs.length}
          </span>
        </button>
      ...
      <ConfirmDialog
        ...
        description={t('library.deleteConfirmDesc', 'Esta ação é irreversível. Os ficheiros gerados serão apagados do disco.')}
