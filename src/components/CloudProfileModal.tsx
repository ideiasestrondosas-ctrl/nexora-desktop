...
export function CloudProfileModal({ open, onClose, editing }: Props) {
  ...
  const handleOAuthConnect = async () => {
    ...
    const clientSecret = String(fields['client_secret'] ?? '').trim() || null;
  ...
  return (
    ...
    >
      <Dialog.Portal>
        ...
        <Dialog.Content
          className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-bg-primary border border-border rounded-2xl w-full max-w-lg z-50 shadow-2xl flex flex-col max-h-[90vh]"
          onEscapeKeyDown={(e) => {
